...
void Solver::loadPlate( Plate* _plate )
{
	plate = _plate;
	//al = plate->rho;
	al = 1;
...
void Solver::setTask( PL_NUM _J0, PL_NUM _tauJ )
{
	...
	rungeKutta = new RungeKutta( eq_num );
	orthoBuilder = new OrthoBuilderGSh();
...
void Solver::calc_nonlin_system( int _x )
{
	if( plate == 0 ){
...
void Solver::dump_check_sol( int fNum )
{
	PL_NUM sum = 0.0;
	PL_NUM h = plate->h;
	PL_NUM a = plate->a;
